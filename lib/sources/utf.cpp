...
std::string UTF::readUTF16str(std::ifstream* input, uint32_t offset) {
...
std::vector<uint16_t> UTF::convert8to16(const std::string input) {
	std::vector<uint8_t> utf8 = std::vector<uint8_t>(input.size() + 1);
	std::vector<uint16_t> utf16 = std::vector<uint16_t>((chrcount(input) + 1) * 2);
	memcpy(utf8.data(), input.c_str(), input.size());
	utf8[input.size()] = '\0';
	nnc_utf8_to_utf16(utf16.data(), (chrcount(input) + 1) * 2, utf8.data(), input.size());

	return utf16;
}

std::string UTF::convert16to8(const std::string& input) {
	size_t outLen = 0;
...
void write_utf8(uint8_t *out, size_t outlen, size_t *outptr, uint32_t cp)
{
	if(cp < 0x80)
	{
		size_t n = *outptr + 1;
...
size_t UTF::nnc_utf16_to_utf8(uint8_t *out, size_t outlen, const uint16_t *in, size_t inlen)
{
	size_t outptr = 0;
...
size_t UTF::nnc_utf8_to_utf16(uint16_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
	size_t outptr = 0;
	for(size_t i = 0; i < inlen; ++i)
	{
